...
class mnnet(object):
    def __init__(self, dim_x, s_size, beta=0.00001, gamma=0.00001, dim_z=50, batch_size=50):
        ...
        self.g_x = self.generator(self.z, self.dim_x)
        ...
        self.s_t_pred = self.student(self.g_x, s_size, 'student', reuse=True)
        self.vars_S = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='student')

        self.loss_G =  - tf.reduce_mean(tf.square(tf.subtract(self.t_pred, self.s_t_pred))) 
        + self.beta*tf.reduce_mean(tf.reduce_sum(tf.square(self.g_x), 1)) + self.gamma*tf.reduce_mean(tf.square(self.s_t_pred))
    ...
    def frac(self, epoch, iteration):
        frac = 1 - (epoch/iteration)      
        return frac
    ...
    def train(self, X_tst, Y_tst, teacher_path, iteration=2000, n_s=10):
        ...
        for epoch in range(iteration):
            frac = self.frac(epoch, iteration)
            self.sess.run(self.train_G, feed_dict={self.z: np.random.randn(self.batch_size, self.dim_z)})
            for i in range(n_s):
                self.sess.run(self.train_S, feed_dict={self.z: np.random.randn(self.batch_size, self.dim_z), 
                                                      self.x: np.random.randn(self.batch_size, self.dim_x), self.frac_S : frac})    
    ...
    def generator(self, z, dim, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            for _ in range(1):
                z = tf.layers.dense(z, 500, activation = tf.nn.tanh)
            o = tf.layers.dense(z, dim)
        return o
